...
async function importJsonToMysql(pool){for(const file of storageFiles){const collection=path.basename(file,'.json'),payload=fs.readFileSync(file,'utf8');JSON.parse(payload);await pool.execute('INSERT INTO raynet_crm_store (collection_name,payload) VALUES (?,?) ON DUPLICATE KEY UPDATE payload=VALUES(payload), updated_at=CURRENT_TIMESTAMP',[collection,payload])}}
function saveStorageConfig(config){const safe={driver:config.driver};if(config.driver==='mysql')Object.assign(safe,{host:config.host,port:config.port,database:config.database,user:config.user,password:config.password,ssl:config.ssl});const temp=storageConfigFile+'.tmp';fs.writeFileSync(temp,JSON.stringify(safe,null,2)+'\n',{mode:0o600});fs.renameSync(temp,storageConfigFile);savedStorageConfig=safe}
async function activateMysql(config,{importLocal=false,persist=false}={}){const pool=await openMysql(config);try{if(importLocal)await importJsonToMysql(pool);else for(const file of storageFiles){const collection=path.basename(file,'.json'),[rows]=await pool.execute('SELECT payload FROM raynet_crm_store WHERE collection_name=?',[collection]);if(rows.length){const payload=String(rows[0].payload);JSON.parse(payload);fs.writeFileSync(file,payload.endsWith('\n')?payload:payload+'\n')}else{const payload=fs.readFileSync(file,'utf8');await pool.execute('INSERT INTO raynet_crm_store (collection_name,payload) VALUES (?,?)',[collection,payload])}}await mysqlWriteQueue;if(mysqlPool)await mysqlPool.end();mysqlPool=pool;storageDriver='mysql';if(persist)saveStorageConfig(config)}catch(error){await pool.end().catch(()=>{});throw error}}
...
async function api(req,res,pathname){
  ...
  if(pathname==='/api/storage-settings'&&req.method==='PUT'){
    if(user.role!=='admin')return json(res,403,{error:'Administrator access required.'});if(process.env.STORAGE_DRIVER||process.env.MYSQL_HOST)return json(res,409,{error:'Storage is managed by server environment variables. Change those values and restart the service.'});const body=await readBody(req);if(body.driver==='json'){await mysqlWriteQueue;if(mysqlPool){await mysqlPool.end();mysqlPool=null}storageDriver='json';saveStorageConfig({driver:'json'});return json(res,200,{ok:true,driver:'json',message:'JSON storage is active. The existing local files have been retained.'})}if(body.driver!=='mysql')return json(res,400,{error:'Choose JSON or MySQL storage.'});const config=mysqlConfigFrom({...body,password:String(body.password||savedStorageConfig.password||'')});try{await activateMysql(config,{importLocal:true,persist:true});return json(res,200,{ok:true,driver:'mysql',migratedCollections:storageFiles.length,message:'MySQL is active and the current JSON records have been imported.'})}catch(error){return json(res,400,{error:`Could not migrate to MySQL: ${error.message}`})}
  }
